...
/* ─── Footer navy family (all derived from hsl 218 58% 20%) ───────
   navyDeep   #0D1C36  hsl(218 62% 13%)  — darkest, for overlays
   navyCore   #152B51  hsl(218 58% 20%)  — footer/navbar exact match
   navyMid    #1F3A6B  hsl(218 55% 27%)
   navyBright #2C4F8C  hsl(218 52% 36%)
   navyPale   #8DAEE2  hsl(218 60% 72%)  — for light text on dark bg
─────────────────────────────────────────────────────────────────── */
...
// Scroll-progress line: footer navy → mid navy → crimson
const ScrollLine = ({ progress }: { progress: any }) => {
  ...
  return (
    <motion.div
      ...
      style={{
        scaleX,
        background: "linear-gradient(90deg, #152B51, #2C4F8C 40%, #b91c1c)",
      }}
    />
  );
};
  ...
  return (
    <>
      ...
      >
        ...
        {/* Base dark veil — footer navy */}
        <div className="absolute inset-0 z-[1] bg-gradient-to-b from-[#0D1C36]/75 via-[#0D1C36]/40 to-[#0D1C36]/85" />
        {/* Left-edge depth — footer navy */}
        <div className="absolute inset-0 z-[2] bg-gradient-to-r from-[#152B51]/65 via-transparent to-transparent" />
        {/* Bottom crimson warmth — brand accent */}
        <div
          className="absolute inset-0 z-[3]"
          style={{ background: "radial-gradient(ellipse 65% 40% at 50% 120%, rgba(185,28,28,0.28), transparent)" }}
        />
        {/* Subtle grid texture */}
        <div
        ...
        >
          ...
          <motion.div style={{ rotateX: tiltX, rotateY: tiltY, transformPerspective: 1100, transformStyle: "preserve-3d" }}>
            <motion.h1
              ...
              style={{
                ...
                textShadow: "0 24px 60px rgba(13,28,54,0.6)",
                letterSpacing: "-0.03em",
              }}
            >
              Shop Smart.
              <br />
              {/* Headline accent: pale navy → crimson — footer navy family sweep */}
              <motion.span
                ...
                style={{
                  ...
                  backgroundClip: "text",
                  backgroundImage: "linear-gradient(135deg, #8DAEE2 0%, #5A8FCC 35%, #f87171 80%, #fca5a5 100%)",
                }}
              >
                Live Better.
              </motion.span>
          ...
          >
            Fresh groceries, household essentials, personal care, bakery items and more — delivered across Lahore.
          </motion.p>
          ...
          >
            {/* Primary CTA — footer navy */}
            <Link to="/shop">
              <motion.button
                whileHover={{ scale: 1.06, y: -3, boxShadow: "0 18px 44px rgba(21,43,81,0.55)" }}
                whileTap={{ scale: 0.97 }}
                className="group inline-flex items-center gap-2 px-7 py-3.5 rounded-full font-bold text-sm text-white"
                style={{
                  background: "linear-gradient(135deg, #2C4F8C, #0D1C36)",
                  fontFamily: FONT,
                  boxShadow: "0 4px 20px rgba(13,28,54,0.50), inset 0 1px 0 rgba(255,255,255,0.12)",
                }}
              >
            ...
            <Link to="/categories">
              <motion.button
                whileHover={{ scale: 1.06, y: -3, backgroundColor: "rgba(185,28,28,0.18)", borderColor: "rgba(220,38,38,0.5)" }}
                whileTap={{ scale: 0.97 }}
