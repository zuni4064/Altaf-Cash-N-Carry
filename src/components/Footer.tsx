...
const FooterLink = ({ to, label }: { to: string; label: string }) => (
  <motion.div whileHover={{ x: 5 }} transition={{ type: "spring", stiffness: 300, damping: 20 }}>
    ...
    >
      <motion.span className="w-0 h-px bg-white/50 group-hover:w-3 transition-all duration-200 inline-block" />
      {label}
    </Link>
  </motion.div>
);
...
const Footer = () => {
  ...
  return (
    /* ── Navy background — matches navbar/sidebar ── */
    <footer
      className="relative text-white mt-16 overflow-hidden"
      style={{ backgroundColor: "hsl(218,58%,20%)" }}
    >

      {/* Subtle blobs — barely visible, just for depth */}
      {[
        { size: 350, left: "-8%",  top: "-20%", opacity: 0.07 },
        { size: 280, left: "75%",  top: "10%",  opacity: 0.06 },
        { size: 200, left: "40%",  top: "60%",  opacity: 0.05 },
      ].map((b, i) => (
        <motion.div
          key={i}
          className="absolute rounded-full bg-white pointer-events-none"
          style={{ width: b.size, height: b.size, left: b.left, top: b.top, opacity: b.opacity, filter: "blur(70px)" }}
      ...
      <div className="container relative z-10 pt-20 pb-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 mb-14">
          ...
          >
            <h4 className="font-bold text-sm tracking-widest uppercase mb-5 flex items-center gap-2 text-white">
              {/* Red accent dot instead of a colored icon */}
              <span className="w-1.5 h-4 rounded-sm bg-red-500 inline-block" />
              Quick Links
            </h4>
          ...
          >
            ...
            <div className="flex flex-col gap-4 text-sm text-white/60">
              <motion.div whileHover={{ x: 4 }} className="flex items-start gap-2.5">
                <MapPin className="h-4 w-4 flex-shrink-0 mt-0.5 text-white/40" />
                <span>66-67 A, Block A, Central Park Society, Lahore</span>
              </motion.div>
              <motion.div whileHover={{ x: 4 }} className="flex items-start gap-2.5">
                <Phone className="h-4 w-4 flex-shrink-0 mt-0.5 text-white/40" />
                <div className="flex flex-col gap-0.5">
                  <a href="tel:03219410035" className="hover:text-white transition-colors">0321-9410035</a>
                  <a href="tel:03212410035" className="hover:text-white transition-colors">0321-2410035</a>
                </div>
              </motion.div>
              <div className="inline-flex items-center gap-2 bg-white/8 border border-white/12 rounded-xl px-3 py-2 text-xs font-semibold self-start">
                <motion.span
        ...
        {/* Divider + copyright */}
        <div className="border-t border-white/12 pt-6 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-white/40">
